                  ...
                  <div className="divide-y divide-fg/5">
                    {earningsByFecha[fecha].map((e, i) => (
                      <div
                        ...
                        className="flex flex-wrap items-center gap-x-4 gap-y-1 px-4 py-2.5 cursor-pointer hover:bg-fg/5 transition-colors"
                      >
                        <span className="w-14 font-mono text-sm font-bold text-fg shrink-0">{e.ticker ?? "—"}</span>
                        <span className="min-w-0 flex-1 truncate font-mono text-xs text-fg/60">{e.empresa ?? e.ticker ?? "—"}</span>
                        {e.momento && (
                          <span className="text-right font-mono text-[10px] text-fg/40 shrink-0">{e.momento}</span>
                        )}
                        <span className="text-right font-mono text-xs tabular-nums text-fg/50 shrink-0">
                          {e.eps_estimado != null ? `EPS Est. $${e.eps_estimado.toFixed(2)}` : ""}
                        </span>
                      </div>
                    ))}
                  </div>
